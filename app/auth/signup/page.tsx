  ...
  useEffect(() => {
    const checkAuthAndRedirect = async () => {
      ...
      try {
        ...
        console.log('User not authenticated, proceeding with sign-up');
      }
      
      setIsChecking(false);
      setIsRedirecting(true);
      // Call without provider to show Hosted UI with all options (includes signup)
      signInWithRedirect();
    };
      ...
      <div className="max-w-md w-full space-y-8 bg-white/15 dark:bg-white/5 backdrop-blur-xl border border-white/25 dark:border-white/10 rounded-3xl p-8 shadow-[0_8px_32px_rgba(255,255,255,0.15),0_8px_32px_rgba(0,0,0,0.15)] dark:shadow-lg dark:shadow-black/5">
        ...
        <div className="mt-8 space-y-6 text-center">
          <p className="text-muted-foreground">
            {isChecking ? 'Checking authentication status...' : 'Redirecting to AuspicesAI sign-up page...'}
          </p>
          <Button
            type="button"
            ...
            size="lg"
          >
            {isChecking ? 'Checking...' : isRedirecting ? 'Redirecting…' : 'Go to Sign Up'}
          </Button>
        </div>
      </div>
    </div>
